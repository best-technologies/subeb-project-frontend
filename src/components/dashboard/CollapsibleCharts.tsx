...
const CollapsibleCharts: React.FC<CollapsibleChartsProps> = ({
  dashboardData,
}) => {
  ...
  const classAverages = (
    ...
    .map((classItem) => ({
      ...
      average: Math.round(classItem.currentEnrolment > 0 ? 75 : 0), // Default average
    }))
    .sort((a, b) => b.average - a.average);

  // Calculate gender performance from real data
  const maleAverage =
    dashboardData?.summary?.totalMale && dashboardData.summary?.totalStudents
      ? Math.round(
          (dashboardData.summary.totalMale /
            dashboardData.summary.totalStudents) *
            100
        )
      : 0;
  const femaleAverage =
    dashboardData?.summary?.totalFemale && dashboardData.summary?.totalStudents
      ? Math.round(
          (dashboardData.summary.totalFemale /
            dashboardData.summary.totalStudents) *
            100
        )
      : 0;
            ...
            <div className="space-y-6">
              ...
              <div className="text-center">
                <p className="text-sm text-gray-300">
                  {dashboardData?.summary?.totalMale || 0} Male •{" "}
                  {dashboardData?.summary?.totalFemale || 0} Female
                </p>
              </div>
            </div>
          </div>
              ...
              {[
                {
                  ...
                  count: topStudents.filter((s) => (s.totalScore || 0) >= 630)
                    .length,
                },
                {
                  range: "80-89",
                  color: "from-blue-500 to-blue-600",
                  count: topStudents.filter(
                    (s) =>
                      (s.totalScore || 0) >= 560 && (s.totalScore || 0) < 630
                  ).length,
                },
                {
                  range: "70-79",
                  ...
                  count: topStudents.filter(
                    (s) =>
                      (s.totalScore || 0) >= 490 && (s.totalScore || 0) < 560
                  ).length,
                },
                {
                  range: "60-69",
                  ...
                  count: topStudents.filter(
                    (s) =>
                      (s.totalScore || 0) >= 420 && (s.totalScore || 0) < 490
                  ).length,
                },
                {
                  range: "Below 60",
                  color: "from-red-500 to-red-600",
                  count: topStudents.filter((s) => (s.totalScore || 0) < 420)
                    .length,
                },
                ...
                <div key={range} className="flex items-center space-x-4">
                  ...
                  <div className="flex-1 bg-white/10 rounded-full h-3">
                    <div
                      ...
                      style={{
                        width: `${(count / topStudents.length) * 100}%`,
                      }}
                    />
                  </div>
                  <div className="w-12 text-right text-sm font-bold text-white">
